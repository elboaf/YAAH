...
timestamp: 2026-09-11T05-18-31Z
slug: src-components-tsx
---
# Critique: src/components.tsx (YAAH main UI)
...
- `design-system-color` (advisory) at components.tsx:316 — `rgba(0,0,0,0.35)` in the ticker fade mask is undocumented in DESIGN.md; legitimate, should be added to the sidecar or ignore list.

**Visual overlays**: not available — no browser automation tool is exposed in this session, and the app requires its backend for meaningful state. No user-visible overlay was produced; deterministic CLI scan above is the machine evidence.

## Overall Impression

The core agent loop is genuinely well-designed — the transparency machinery (ticker, trace, ask card, status line) is distinctive and legible. What drags the score down is everything the public-distribution claim now makes load-bearing: silent failures, native dialogs, dead-end errors, and undiscoverable features. The single biggest opportunity: make failure states as well-designed as the working state.

## What's Working

1. **Ticker→trace collapse** — loud while working, one quiet line when done. It solves the "agent spam" problem every agent UI has, and the fade mask keeps it bounded without a scrollbar.
2. **The ask card** — a distinct orange channel with option chips, a dashed "Something else…" escape hatch, and a pulsing marker. "Busy" (amber) and "waiting on you" (orange) never blur. Best-in-class human-in-the-loop clarity.
...
1. **[P1] Silent attachment failures** — `addImageFile` drops images >5MB or non-images with no feedback; `readDroppedFiles` silently skips text files >200KB and binary-looking files. The user believes the agent received the content.
   **Why it matters**: the agent then works from incomplete context, and the user has no way to know the drop happened — a trust failure in a product whose thesis is transparency.
   **Fix**: on drop/paste/attach, reject with an inline chip or toast: "image.png skipped — 7.2 MB exceeds the 5 MB limit."
   **Suggested command**: `/impeccable harden`

2. **[P1] Native browser dialogs break the crafted surface** — `confirm()` for file/conversation delete, `prompt()` for the system-prompt override, `alert()` for export/delete failures — while the app ships its own modal system (PreviewModal, SettingsModal). SettingsModal also lacks the Esc handler PreviewModal has.
...
**Jordan (First-Timer)**: "/s" undiscoverable; "md↓" and "sys" row actions are unlabeled jargon; the no-provider state hides inside a select option ("No provider configured — open Settings"); workspace field expects a path with no guidance. Will abandon at first-run configuration.
...
- Empty-state copy ("Work in progress streams as a live ticker, then collapses to a one-line trace") explains mechanics in developer terms; it should teach the first action instead.
...
- CopyButton flashes "copied!" with no aria-live announcement; the status dot's meaning is carried by its adjacent text label (good), but the dot itself has no accessible name.
- The status line shows raw status strings ("thinking", "running-tool") — machine voice is right for this product, but "running-tool" leaks an internal enum spelling.
